...
const PurchaseModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const loadData = async () => {
    try {
      ...
      setSuppliers(sRes.data.parties || []);
  ...
  const handleProductSelect = (index, prodId) => {
    ...
    if (prod) {
      newItems[index] = {
        ...
        total: (newItems[index].quantity || 1) * (prod.purchasePrice || 0),
      };
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (numPaid > 0 && !accountId) {
      setError('Please select an account for the paid amount');
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Record Purchase Invoice (Kharidari)" size="lg">
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ padding: '0.65rem 0.85rem', backgroundColor: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.8rem' }}>
          <span style={{ color: 'var(--text-secondary)' }}>Purchasing Brother:</span>
            ...
            >
              ...
              {suppliers.map((s) => (
                <option key={s._id} value={s._id}>
                  {s.name} (Payable: {formatCurrency(Math.abs(s.currentBalance))})
                </option>
              ))}
            </select>
          </div>
        ...
        <div style={{ marginBottom: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 700 }}>Items to Stock In</span>
            ...
            <table className="custom-table">
              <thead>
                <tr>
                  ...
                  <th style={{ width: '20%' }}>Cost Rate (Rs.)</th>
        ...
        <div style={{ padding: '1rem', backgroundColor: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', marginBottom: '1rem', border: '1px solid var(--border-color)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <div>
              <label className="form-label">Net Bill</label>
            ...
            <div>
              <label className="form-label">Payable Due (Udhaar)</label>
        ...
        <div className="form-group">
          <label className="form-label">Notes / Supplier Bill #</label>
          <input
            type="text"
            className="form-control"
            placeholder="e.g. Supplier Gate Pass #102 / Mill delivery"
            value={notes}
        ...
        <div className="modal-footer" style={{ margin: '1rem -1.5rem -1.5rem -1.5rem' }}>
          ...
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? 'Recording...' : 'Save Purchase Bill'}
          </button>
        </div>
      </form>
    </Modal>
